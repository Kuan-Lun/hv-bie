...
`hv-bie` 是純離線 HentaiVerse battle HTML parser library。公開入口是
`parse_snapshot(html: str) -> BattleSnapshot`，不得加入 browser、network、
account或 live battle責任。

- `parse_snapshot()`協調 player vitals、buffs、abilities、monsters、combat
  log與items六類 parser；各 parser維持單一 DOM section責任。
- 缺少或不完整 DOM section時填入安全預設並附加 diagnostic warning，不得
  因缺少 section而改成直接 raise。
- `hv_bie/types/`的 immutable dataclasses及 `BattleSnapshot.as_dict()`、
  `to_json()`是公開資料契約。
- CSS sprite decoding、player 414px與monster 120px bar換算、attribute/style
  fallback及system-monster分類都必須保有 regression coverage。
- 新增、變更或移除功能、欄位、公開型別或 API行為時，同一變更必須同步
  `SRS.md`與`API_SPEC.md`；兩者維持繁體中文並與實作一致。
- 測試使用離線真實 HTML fixtures，涵蓋 unit、snapshot serialization整合與
  既有每次 parse低於50ms的 performance contract。
- full gate必須執行 deterministic pytest、sdist/wheel build，以及從新建wheel
  path import `hv_bie`的 smoke test。
